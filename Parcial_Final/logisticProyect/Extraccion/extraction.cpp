...
//oFichero(lista_costos, "fichero_costo.txt");
...
// función para pasar de lista a fichero
/*void Extraction::ListaToFichero(std::list<double> lcostos ,std::string nombre_file){
    // Se nombra el fichero
    std::ofstream fichero_salida(nombre_file);

    // si el fichero salida esta abierto, guarda datos
    if(fichero_salida.is_open()){
        fichero_salida << lcostos << "\n";
    }
}
*/

/*void Extraction::List_to_file(std::list<float> lista, std::string nombre_fichero){
    //Se nombra el fichero
    ...
    // Se entrega la copia a un fichero
    std::copy(lista.begin(),lista.end(),iterador_salida);
